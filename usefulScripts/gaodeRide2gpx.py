...
def create_gpx(points, start_ts, output_dir):
    gpx = ET.Element('gpx', version="1.1", creator="RideGPXExporter", xmlns="http://www.topografix.com/GPX/1/1")
    trk = ET.SubElement(gpx, 'trk')
    name = ET.SubElement(trk, 'name')
    name.text = f"Ride {start_ts}"
...
def main():
    ...
    output_dir = r"gpx_output"

    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("SELECT RIDE_POI FROM ride_table")
    ...
    for row in rows:
        ...
        if not a:
            continue
        try:
            point_list = json.loads(json.loads(a)["RidePonitList"])
        ...
        for point in point_list:
            x = point.get("RidePonitX")
            y = point.get("RidePonitY")
            ...
            lon_wgs, lat_wgs = gcj02_to_wgs84(lon_gcj, lat_gcj)  # 坐标系转换！
